...
/* Reserve 480*272*4 bytes for display frame buffer.*/
/* Reserve an additional 4096 bytes so that the frame buffer can be
   aligned on the SDRAM's burst boundary. */
#define CONFIG_SYS_MALLOC_LEN		(CONFIG_ENV_SIZE + 256 * 1024 + 480*272*4 + 4096)

/* Reserve 2^23 bytes for rotator buffer. */
/* The CONFIG_PRAM constant is given in kB */
#define CONFIG_PRAM (1 << 13)
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"fpga_led_pio=0xff249180\0" \
	"splashload=sf probe ${qspiloadcs}; sf read ${splashloadaddr} ${qspisplashimageaddr} ${splashimagesize}\0" \
	"splashboot=run qspifpga; run bridge_enable_handoff; run splashload; go 0xC1000CD; mw ${fpga_led_pio} 0x02\0" \
	"fpga=0\0" \
