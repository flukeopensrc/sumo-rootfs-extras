...
static void setenv_ethaddr_eeprom(void)
{
	...
	/* Fetch factory programmed Ethernet address from Atmel AT24MAC402 for Nighthawk */
	chip = 0x58;		/* slave ID for Atmel AT24MAC402 */
	addr = 0x9a;		/* Address offset of unique MAC adddress */
	alen = 1;		/* Atmel AT24MAC402 uses 1 byte addressing */
	i2c_read(chip, addr, alen, enetaddr, linebytes);

	/* Note that Atmel doesn't swap endian of MAC address. */
	if (is_valid_ether_addr(enetaddr))
		eth_setenv_enetaddr("eth1addr", enetaddr);
	else
		puts("Skipped ethaddr assignment due to invalid "
			"EMAC address in Atmel AT24MAC402\n");
}
