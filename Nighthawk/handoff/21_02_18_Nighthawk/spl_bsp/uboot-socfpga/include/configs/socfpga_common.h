...
/* Reserve 800x480*4 bytes for display frame buffer.*/
/* Reserve an additional 4096 bytes so that the frame buffer can be
   aligned on the SDRAM's burst boundary. */
#define CONFIG_SYS_MALLOC_LEN		(CONFIG_ENV_SIZE + 256 * 1024 + 800*480*4)
...
#else
#define CONFIG_BOOTCOMMAND "run qspifpga; run bridge_enable_handoff; run qspiload; run swapmac; run qspiboot"
#endif
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"nandboot=setenv bootargs " CONFIG_BOOTARGS \
		...
		"bootz ${loadaddr} - ${fdtaddr}\0" \
	"qspisplashimageaddr=0xE00000\0" \
	"splashimagesize=0x180000\0" \
	"splashloadaddr=0xC100000\0" \
	"splashload=sf probe ${qspiloadcs}; sf read ${splashloadaddr} ${qspisplashimageaddr} ${splashimagesize}\0" \
	"splashboot=run qspifpga; run bridge_enable_handoff; run splashload; go 0xC1000CD\0" \
	"swapmac=setenv temp ${ethaddr}; setenv ethaddr ${eth1addr}; setenv eth1addr ${temp}\0 " \
	"fpga=0\0" \
